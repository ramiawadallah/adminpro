        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    {{ ucfirst(config('adminpro.prefix')) }} List
                    <span class="float-right">
                        <a href="{{route('admin.register')}}" class="btn btn-sm btn-success">New {{ ucfirst(config('adminpro.prefix')) }}</a>
                    </span>
                </div>
                <div class="card-body">
    @include('adminpro::message')
                    <ul class="list-group">
                        ...
                        <p>No {{ config('adminpro.prefix') }} created Yet, only super {{ config('adminpro.prefix') }} is available.</p>
                        @endif
                    </ul>
                </div>
            </div>
